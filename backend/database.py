...
class Conversation(Base):
    ...
    created_at: Mapped[int] = mapped_column(Integer, nullable=False, default=lambda: int(datetime.now().timestamp()))
    updated_at: Mapped[int] = mapped_column(Integer, nullable=False, default=lambda: int(datetime.now().timestamp()))
...
class UsageLog(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    conversation_id: Mapped[str] = mapped_column(String, ForeignKey("conversations.id", ondelete="SET NULL"), nullable=True)
    model: Mapped[str] = mapped_column(String, nullable=False)
    ...
    timestamp: Mapped[int] = mapped_column(Integer, nullable=False, default=lambda: int(datetime.now().timestamp()))
